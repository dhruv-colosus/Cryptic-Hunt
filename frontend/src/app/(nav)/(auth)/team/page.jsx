        ...
        <>
            <div className="flex justify-center items-center h-screen bg-background">
                <div className="w-192">
                    ...
                    <h1 className="sm:text-2xl block text-center font-heading text-base mt-12">CODE FOR YOUR TEAM IS - <span className="text-main">{teamData.teamCode}</span></h1>
                    <h1 className="sm:text-xl block sm:mx-0 mx-4 text-center font-sub text-base">Only share this code with people you want in your team.</h1>
                    <div className="w-192 p-6 mt-12 mb-16">
                        <h1 className="sm:text-2xl block text-center font-heading text-base">Team Members</h1>
                        <div className="grid grid-cols-2 w-full mt-12 md:gap-8 gap-2">
                            {
                                teamData.members.map((member, index) => {
                                    console.log(member)
                                    return <><div key={member._id} className="sm:text-xl text-sm text-main font-minecraftia">
                                        {index + 1}) {member.name}
                                    </div>
                                        {member._id === teamData.teamOwner ?
                                            <div className="sm:text-lg sm:ml-52 ml-24 text-base text-sub "><FaCrown size={28} /></div>
                                            :
                                            user._id === teamData.teamOwner ?
                                                <div className="sm:text-lg sm:ml-52 ml-24 text-base text-sub "><button
                                                    onClick={
                                                        async () => {
                                                    ...
                                                    disabled={member._id === teamData.teamOwner || kickUserMutation.isLoading}
                                                >Kick</button></div> : <div></div>}
                                    </>
                                }
                                )
                            }
