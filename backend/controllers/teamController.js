...
const createNewTeam = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        await User.findByIdAndUpdate(user._id, { $set: { teamId: newTeam._id } }, { session });
...
const getTeam = asyncHandler(async (req, res) => {
    ...
    const existingTeam = await Team.findById(teamId).populate('members', '-password')
...
const addMember = asyncHandler(async (req, res) => {
    ...
    if (existingTeam.members.find(id => id === memberId)) {
        return res.status(400).json({ 
...
const kickMember = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        await User.findByIdAndUpdate(memberId, { $set: { teamId: null } }, { session });
